...
The Agent page also accepts an optional `feishu_webhook` (a Feishu custom bot URL), and each channel has a `notify_feishu` switch. When a build finishes successfully or fails, and the switch is on, the Agent posts the result (project, channel, branch, commit, duration, failure reason) straight to that bot without depending on the Manager.

The Agent configuration file is created after the first project is saved in the Agent page. A project contains `id`, `name`, `path`, `unity_path`, `log_path`, `default_branch`, and an optional `schedule` (`enabled`, `time` as `HH:MM`) for Agent-side daily builds; each channel contains `name`, read-only `platform` (`Android`, `iOS`, or `OpenHarmony`), `switch_to` (`android_dev`, `android_release`, `ios_dev`, `ios_release`, `openharmony_dev`, or `openharmony_release`), `branch_filter`, `build_method`, and an optional `sync_mainline` switch. A scheduled build uses the project's first enabled channel and the same branch the Manager preselects, runs on the Agent's local clock even while the Manager is offline, and is skipped when the project is already building. With `sync_mainline` enabled, a suffixed branch such as `dev/9-2-26_鸿蒙` fetches its mainline `dev/9-2-26`, merges the updated origin mainline into the branch as a local merge commit before packing, records that merge commit as the task revision for traceability, and fails the build on merge conflicts.
